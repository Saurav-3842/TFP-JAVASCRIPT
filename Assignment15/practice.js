const data = {
  ...
  const addparaElement = (card,text)=>{
    // key=text;
    const paraElem = document.createElement("p"); 
    paraElem.innerHTML=text;
    card.appendChild(paraElem);
  }
  const addimgElement = (card,img)=>{
    const imgElem=document.createElement("img");
    imgElem.src=img;
    card.appendChild(imgElem);
  }
  const adddeleteBtn=(card,text,customclass,id)=>{
    const dtlbtn = document.createElement("button");
    dtlbtn.innerHTML=text;
    dtlbtn.classList.add(customclass);
    const cb=()=>{
      deleteCard(id);
    }
    dtlbtn.addEventListener("click",cb);
    card.appendChild(dtlbtn);
  }
  const createCard= (parent,{id,name,ingredients,instructions,prepTimeMinutes,cookTimeMinutes,servings,difficulty,cuisine,caloriesPerServing,tags,userId,image,rating,reviewCount,mealType},res1,cardindex)=>{
    const card =document.createElement("div");
    card.className = 'card';
    let key1=Object.keys(res1)[0];
    addparaElement(card,id);
    addparaElement(card,name);
    addparaElement(card,ingredients);
    addparaElement(card,instructions);
    addparaElement(card,prepTimeMinutes);
    addparaElement(card,cookTimeMinutes);
    addparaElement(card,servings);
    addparaElement(card,difficulty);
    addparaElement(card,cuisine);
    addparaElement(card,caloriesPerServing);
    addparaElement(card,tags);
    addparaElement(card,userId);
    addimgElement(card,image);
    addparaElement(card,rating);
    addparaElement(card,reviewCount);
    addparaElement(card,mealType);
    adddeleteBtn(card,"Delete","dlt-btn",cardindex);
    parent.appendChild(card);
  }

  const createUI = (recipe)=>{
    const tempContainer = new DocumentFragment();
    
    recipe.forEach((res,idx)=> createCard(tempContainer,res,res,idx));
